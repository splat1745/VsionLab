...
function switchProjectTab(tabId) {
    ...
    if (tabId === 'annotate') {
        ...
        loadDatasetTab(state.currentProject.id);
    } else if (tabId === 'versions') {
        loadVersionsTab(state.currentProject.id);
    } else if (tabId === 'train') {
...
// ============== Dataset Tab Logic ==============
async function loadDatasetTab(projectId) {
    const grid = document.getElementById('dataset-images-grid');
    if (!grid) return;
    
    grid.innerHTML = '<div style="padding: 20px; text-align: center;">Loading images...</div>';
    
    try {
        // Reuse existing loadImages logic but target the grid
        const response = await fetch(\\/projects/\/images\);
        const images = await response.json();
        
        // Store in state for other uses
        state.images = images;
        
        renderDatasetGrid(images);
    } catch (error) {
        console.error(error);
        grid.innerHTML = '<div style="padding: 20px; text-align: center; color: var(--error);">Failed to load images</div>';
    }
}
...
function renderDatasetGrid(images) {
    ...
    if (images.length === 0) {
        grid.innerHTML = \
            <div class="empty-state">
                ...
                <p>Upload images to populate the dataset.</p>
            </div>
        \;
        return;
    }
    ...
    grid.innerHTML = images.map((img, index) => \
        <div class="image-card" onclick="openAnnotationView(\)">
            <img src="\/images/\/thumbnail" loading="lazy" alt="\">
            \
            <div class="image-card-overlay">
                <span style="font-size: 0.8rem; color: white; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%;">\</span>
            </div>
        </div>
    \).join('');
    
    const countEl = document.getElementById('dataset-selected-count');
    if (countEl) countEl.textContent = \\ images\;
}

// ============== Versions Tab Logic ==============
async function loadVersionsTab(projectId) {
    const list = document.getElementById('versions-sidebar-list');
    if (!list) return;
    
    list.innerHTML = '<div style="padding: 20px; text-align: center;">Loading...</div>';
    
    try {
        const res = await fetch(\\/projects/\/datasets\);
        const datasets = await res.json();
        renderVersionsList(datasets);
    } catch (e) {
        console.error(e);
        list.innerHTML = '<div style="padding: 20px; text-align: center; color: var(--error);">Failed to load versions</div>';
    }
}

function renderVersionsList(datasets) {
    const list = document.getElementById('versions-sidebar-list');
    if (!list) return;
    
    if (datasets.length === 0) {
        list.innerHTML = '<div class="empty-state" style="padding: 20px;">No versions</div>';
        return;
    }
    
    list.innerHTML = datasets.map(ds => \
        <div class="version-card-sidebar" onclick="selectVersion(\)" style="padding: 12px; border: 1px solid var(--border-color); margin-bottom: 8px; border-radius: 6px; cursor: pointer; background: var(--bg-secondary); transition: all 0.2s;">
            <div style="font-weight: 600; margin-bottom: 4px;">\</div>
            <div style="display: flex; gap: 8px; font-size: 0.75rem; color: var(--text-secondary);">
                <span style="background: var(--bg-tertiary); padding: 2px 6px; border-radius: 4px;">\</span>
                <span>\ images</span>
            </div>
        </div>
    \).join('');
    ...
    if (datasets.length > 0) {
        selectVersion(datasets[0].id);
    }
}

window.selectVersion = async function(datasetId) {
    const panel = document.getElementById('version-details-panel');
    if (!panel) return;
    
    // Highlight selected
    document.querySelectorAll('.version-card-sidebar').forEach(el => {
        el.style.borderColor = 'var(--border-color)';
        el.style.background = 'var(--bg-secondary)';
    });
    // (Ideally we'd pass the element or find it by ID, but for simplicity just highlight clicked)
    
    panel.innerHTML = '<div style="padding: 40px; text-align: center;">Loading details...</div>';
    
    try {
        // Fetch dataset details (we might need a specific endpoint or just use what we have)
        // For now, let's simulate fetching details
        
        // Mock data for visualization
        const metrics = {
            map: (85 + Math.random() * 10).toFixed(1),
            precision: (88 + Math.random() * 10).toFixed(1),
            recall: (80 + Math.random() * 10).toFixed(1)
        };
        ...
        panel.innerHTML = \
            <div style="padding: 20px;">
                <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 20px;">
                    <div>
                        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                            <span style="background: #000; color: #fff; padding: 2px 8px; border-radius: 4px; font-size: 0.8rem; font-weight: bold;">v1</span>
                            <h2 style="margin: 0;">Dataset Version \</h2>
                        </div>
                        <p style="color: var(--text-secondary);">Generated on \</p>
                    </div>
                ...
                <div class="metrics-card" style="padding: 24px; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 8px; margin-bottom: 24px;">
                    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 20px;">
                        <h3 style="margin: 0; font-size: 1rem;">Model Metrics</h3>
                        <i class="fas fa-chart-bar" style="color: var(--accent-primary);"></i>
                    </div>
                    
                    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px;">
                        <div>
                            <div style="font-size: 0.85rem; color: var(--text-secondary); margin-bottom: 8px;">mAP@50</div>
                            <div style="font-size: 2rem; font-weight: 700; color: var(--text-primary);">\%</div>
                            <div style="height: 6px; background: var(--bg-tertiary); border-radius: 3px; margin-top: 8px; overflow: hidden;">
                                <div style="height: 100%; width: \%; background: var(--accent-primary);"></div>
                            </div>
                        </div>
                        <div>
                            <div style="font-size: 0.85rem; color: var(--text-secondary); margin-bottom: 8px;">Precision</div>
                            <div style="font-size: 2rem; font-weight: 700; color: var(--text-primary);">\%</div>
                            <div style="height: 6px; background: var(--bg-tertiary); border-radius: 3px; margin-top: 8px; overflow: hidden;">
                                <div style="height: 100%; width: \%; background: var(--success);"></div>
                            </div>
                        </div>
                        <div>
                            <div style="font-size: 0.85rem; color: var(--text-secondary); margin-bottom: 8px;">Recall</div>
                            <div style="font-size: 2rem; font-weight: 700; color: var(--text-primary);">\%</div>
                            <div style="height: 6px; background: var(--bg-tertiary); border-radius: 3px; margin-top: 8px; overflow: hidden;">
                                <div style="height: 100%; width: \%; background: var(--warning);"></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px;">
                    <div style="padding: 20px; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 8px;">
                        <h3 style="font-size: 1rem; margin-bottom: 16px;">Configuration</h3>
                        <div style="display: grid; grid-template-columns: auto 1fr; gap: 12px 24px; font-size: 0.9rem;">
                            <div style="color: var(--text-secondary);">Model Type:</div>
                            <div style="font-weight: 500;">YOLOv8 Small</div>
                            
                            <div style="color: var(--text-secondary);">Image Size:</div>
                            <div style="font-weight: 500;">640x640</div>
                            
                            <div style="color: var(--text-secondary);">Batch Size:</div>
                            <div style="font-weight: 500;">16</div>
                            
                            <div style="color: var(--text-secondary);">Epochs:</div>
                            <div style="font-weight: 500;">100</div>
                        </div>
                    </div>
                    
                    <div style="padding: 20px; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 8px;">
                        <h3 style="font-size: 1rem; margin-bottom: 16px;">Dataset Info</h3>
                        <div style="display: grid; grid-template-columns: auto 1fr; gap: 12px 24px; font-size: 0.9rem;">
                            <div style="color: var(--text-secondary);">Total Images:</div>
                            <div style="font-weight: 500;">1,240</div>
                            
                            <div style="color: var(--text-secondary);">Train Set:</div>
                            <div style="font-weight: 500;">868 (70%)</div>
                            
                            <div style="color: var(--text-secondary);">Valid Set:</div>
                            <div style="font-weight: 500;">248 (20%)</div>
                            
                            <div style="color: var(--text-secondary);">Test Set:</div>
                            <div style="font-weight: 500;">124 (10%)</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        console.error(e);
        panel.innerHTML = '<div style="padding: 20px; text-align: center; color: var(--error);">Failed to load details</div>';
    }
};
